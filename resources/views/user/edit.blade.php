        ...
        <div class="card-body bg-light-subtle p-4">
            <form action="{{ route('users.update', $user->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label fw-semibold small">Password <small class="text-muted">(leave empty to keep current)</small></label>
                            <input type="password" name="password" class="form-control rounded-3 shadow-sm border-0 bg-white" placeholder="•••">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold small">Confirm Password</label>
                            <input type="password" name="password_confirmation" class="form-control rounded-3 shadow-sm border-0 bg-white">
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <div class="d-flex flex-wrap gap-4 ms-2">
                        @php $roles = ['admin', 'sales', 'report', 'Sudirman Park']; @endphp
                        @foreach ($roles as $role)
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="role" id="{{ $role }}" value="{{ $role }}" {{ $user->role === $role ? 'checked' : '' }}>
                                <label class="form-check-label fw-semibold small" for="{{ $role }}">{{ ucfirst($role) }}</label>
                            </div>
                        @endforeach
                    </div>
                </div>
